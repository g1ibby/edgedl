//! Generic HWCN aligned SIMD kernels for ESP32-S3 TIE728.
//!
//! This module provides the aligned generic spatial (HWCN) TIE728 implementation.
//! It supports arbitrary kernel sizes and dilations, using the `tie728_s8_conv2d_hwcn`
...
pub fn run(
    ...
    // Check alignment and multiples of 16
    if in_meta.shape.c % 16 != 0 || out_meta.shape.c % 16 != 0 {
        return ExecOutcome::Fallback;
    }
    ...
    let mut args = build_kernel_args_hwcn(plan, arena, spec, node, kh, kw);

    // Prepare per-channel scales/bias if needed
        ...
        crate::model::ParamExponents::PerTensor(w_e_tensor) => {
            ...
            if shift_i > 0 {
                ...
                if sbits < 0 || sbits > 14 {
                    #[cfg(feature = "trace")]
                    crate::ne_warn!(
                        "hwcn_aligned id={} SILENT_SCALAR: sbits={} out of range",
                        node.output,
                        sbits
                    );
                ...
                if has_bias {
                    ...
                    for i in 0..oc {
                        ...
                        unsafe {
                            *bias_ptr.add(i) = adj.clamp(i16::MIN as i32, i16::MAX as i32) as i16;
                        }
                    }
        ...
        crate::model::ParamExponents::PerChannel(w_exps) => {
            ...
            for i in 0..oc {
                ...
                if sbits < 0 || sbits > 14 {
                    #[cfg(feature = "trace")]
                    crate::ne_warn!(
                        "hwcn_aligned id={} SILENT_SCALAR: sbits={} out of range (oc={})",
                        node.output,
                        sbits,
                        i
                    );
    ...
    let kh_usize = kh as usize;
    let kw_usize = kw as usize;
    ...
    // OHWI strides: offset = oc * (kh * kw * ic) + ky * (kw * ic) + kx * ic + ic_idx
    let ohwi_stride_oc = kh_usize * kw_usize * ic_usize;
    let ohwi_stride_ky = kw_usize * ic_usize;
    let ohwi_stride_kx = ic_usize;
    // BlockedN16HWC16 strides: block_size, and per-position stride
    // (These were used by old scalar fallback, now kept for potential OHWI fallback path)
    let _blocked_block_size = kh_usize * kw_usize * ic_usize * 16;
    let _blocked_stride_ky = kw_usize * ic_usize * 16;
    ...
    unsafe {
        ...
        let kh_val = args.kh as isize;
        let kw_val = args.kw as isize;
        ...
        for oy in 0..oh {
            for ox in 0..ow {
                ...
                // Check if the window is fully in bounds
                // Window extent: [origin, origin + (k-1)*dil]
                let max_y = in_y_origin + (kh_val - 1) * dil_h;
                let max_x = in_x_origin + (kw_val - 1) * dil_w;
                ...
                if in_y_origin >= 0 && in_x_origin >= 0 && max_y < ih && max_x < iw {
                    ...
                    // Compute effective filter dimensions for this border pixel
                    ...
                    if eff_kh > 0 && eff_kw > 0 && is_blocked {
                        // Create modified args for border pixel SIMD processing
                        // BlockedN16HWC16 layout: weight stride is ic * 16 per position
                        let filter_c_n_offset = ic * 16;

                        // Compute filter pointer offset to skip clipped rows/cols at start
                        // Offset = (fh_start * kw + fw_start) * ic * 16
                        let filter_start_offset =
                            (fh_start * kw_val + fw_start) * filter_c_n_offset;

                        // filter_y_offset: skip columns at end of each row (tail width clipping)
                        // = (original_kw - effective_kw) * ic * 16
                        let border_filter_y_offset = (kw_val - eff_kw) * filter_c_n_offset;

                        // filter_n_offset: skip rows after processing effective_kh rows (tail
                        // height clipping) = (original_kh - effective_kh) *
                        // original_kw * ic * 16
                        let border_filter_n_offset = (kh_val - eff_kh) * kw_val * filter_c_n_offset;
                        ...
                        // dilation_y_offset = ic * (dil_h * iw - 1 - (eff_kw - 1) * dil_w)
                        let border_dilation_y_offset = ic * (dil_h * iw - 1 - (eff_kw - 1) * dil_w);

                        // Create border args by copying and modifying relevant fields
                        // Use ptr::read for bitwise copy since KernelArgs doesn't impl Copy
                        ...
                        // dilation_x_offset stays the same: (dil_w * ic) - ic
                        ...
                        // (This path is rarely used - most models use BlockedN16HWC16)
                        for oo in 0..oc {
                            let mut acc32: i32 = 0;
        ...
        #[cfg(feature = "trace")]
        {
            ...
            crate::ne_info!(
                "hwcn_aligned id={}: simd={}px ({}cy, {}cy/px), pad={}px ({}cy, {}cy/px)",
                node.output,
                simd_pixels,
                simd_cycles,
                simd_cy_per_px,
...
#[cfg(all(feature = "simd-s3", target_arch = "xtensa"))]
unsafe extern "C" {
    fn dl_tie728_s8_conv2d_hwcn(output_ptr: *mut i8, input_ptr: *const i8, args: *const KernelArgs);
}

fn build_kernel_args_hwcn(
    plan: &PlannedArena,
    arena: &mut Arena<'_>,
    spec: &ModelSpec,
    node: &Conv2dSpec,
    ...
    // Calculate HWCN specific fields
    args.c_div_x_1 = (ic / 16) - 1;
    args.c_remainder = 0; // Aligned path assumes multiple of 16
    args.oc_tiles = oc / 16;
    args.oc_tail = 0; // Aligned path assumes multiple of 16

    // Dilation offsets
    // dilation_x_offset = (dilation_x * ic - ic)
    // dilation_y_offset = ic * (dilation_y * iw - 1 - (kw - 1) * dilation_x)
    let dil_x = args.dil_w;
    let dil_y = args.dil_h;
    args.dilation_x_offset = (dil_x * ic) - ic;
    args.dilation_y_offset = ic * (dil_y * iw - 1 - (kw - 1) * dil_x);

    // Filter offsets for hwc16 macro (stored in q6 vector register slots 1 and 2)
    // For aligned interior pixels with full filter: both are 0
    // Formula from esp-dl: filter_y_offset = (kw - effective_kw) * filter_c_n_offset
    // Since we only process interior pixels, effective_kw == kw, so offset = 0
    // Same logic applies to filter_n_offset.
