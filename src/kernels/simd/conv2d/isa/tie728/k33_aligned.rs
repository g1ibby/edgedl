//! 3x3 (33cn) aligned SIMD kernels for ESP32-S3 TIE728.
//!
//! This module provides the aligned 3x3 (OHWI) TIE728 implementation.
//! It mirrors the 1x1 aligned implementation but handles the 3x3 spatial kernel
//! using the `tie728_s8_conv2d_33c16` macro and dilation offsets.
...
pub fn run(
    ...
    // (Router should have handled this, but safe to double check)
    ...
    let mut args = build_kernel_args_k33_aligned(plan, arena, spec, node);

    // Prepare per-channel scales/bias if needed (same logic as k11_aligned)
        ...
        crate::model::ParamExponents::PerTensor(w_e_tensor) => {
            ...
            if shift_i > 0 {
                ...
                if sbits < 0 || sbits > 14 {
                    #[cfg(feature = "trace")]
                    crate::ne_warn!(
                        "k33_aligned id={} SILENT_SCALAR: sbits={} out of range",
                        node.output,
                        sbits
                    );
        ...
        crate::model::ParamExponents::PerChannel(w_exps) => {
            ...
            for i in 0..oc {
                ...
                if sbits < 0 || sbits > 14 {
                    #[cfg(feature = "trace")]
                    crate::ne_warn!(
                        "k33_aligned id={} SILENT_SCALAR: sbits={} out of range (oc={})",
                        node.output,
                        sbits,
                        i
                    );
    ...
    // OHWI strides for 3x3: offset = oc * 9 * ic + ky * 3 * ic + kx * ic + ic_idx
    let ohwi_stride_oc = 9 * ic_usize;
    let ohwi_stride_ky = 3 * ic_usize;
    let ohwi_stride_kx = ic_usize;
    // BlockedN16HWC16 strides for 3x3
    ...
    unsafe {
        ...
        for oy in 0..oh {
            for ox in 0..ow {
                ...
                if in_y_origin >= 0 && in_x_origin >= 0 && max_y < ih && max_x < iw {
                    ...
                    // Border pixel: use HWCN SIMD with reduced filter dimensions
                    // (k33 kernel only works for exactly 3x3, so we fall back to general hwcn)
                    #[cfg(feature = "trace")]
                    {
                        padding_pixels += 1;
                    }
                    ...
                    if eff_kh > 0 && eff_kw > 0 && is_blocked {
                        // Use HWCN SIMD kernel with reduced filter dimensions
        ...
        #[cfg(feature = "trace")]
        {
            ...
            crate::ne_info!(
                "k33_aligned id={}: simd={}px ({}cy, {}cy/px), pad={}px ({}cy, {}cy/px)",
                node.output,
                simd_pixels,
                simd_cycles,
                simd_cy_per_px,
...
#[cfg(all(feature = "simd-s3", target_arch = "xtensa"))]
unsafe extern "C" {
    fn dl_tie728_s8_conv2d_33cn(output_ptr: *mut i8, input_ptr: *const i8, args: *const KernelArgs);
    // HWCN kernel for border pixels where effective filter < 3x3
    fn dl_tie728_s8_conv2d_hwcn(output_ptr: *mut i8, input_ptr: *const i8, args: *const KernelArgs);
}

fn build_kernel_args_k33_aligned(
    plan: &PlannedArena,
    arena: &mut Arena<'_>,
    spec: &ModelSpec,
    node: &Conv2dSpec,
    ...
    // dilation_y_offset = ic * (dilation_y * iw - 1 - 2 * dilation_x)
