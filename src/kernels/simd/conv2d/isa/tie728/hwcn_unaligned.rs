//! Generic HWCN unaligned SIMD kernels for ESP32-S3 TIE728.
//!
//! This provides the unaligned generic spatial (HWCN) implementation which handles
//! input/output misalignment, IC remainder, and OC tails by reusing the shared
//! unaligned 1×1 inner microkernels and epilogues.
...
pub fn run(
    ...
    // Preconditions: NHWC N==1; supported weight layouts
    ...
    // Build the base args (pointers, shapes, strides/dilations/pads, layout)
    let mut args = build_kernel_args_hwcn_unaligned(plan, arena, spec, node, kh, kw);

    // Prepare per-channel/per-layer quantization epilogue resources
        ...
        crate::model::ParamExponents::PerTensor(w_e_tensor) => {
            ...
            if shift_i > 0 {
                ...
                if sbits < 0 || sbits > 14 {
                    #[cfg(feature = "trace")]
                    crate::ne_warn!(
                        "hwcn_unaligned id={} SILENT_SCALAR: sbits={} out of range",
                        node.output,
                        sbits
                    );
        ...
        crate::model::ParamExponents::PerChannel(w_exps) => {
            ...
            for i in 0..oc {
                ...
                if sbits < 0 || sbits > 14 {
                    #[cfg(feature = "trace")]
                    crate::ne_warn!(
                        "hwcn_unaligned id={} SILENT_SCALAR: sbits={} out of range (oc={})",
                        node.output,
                        sbits,
                        i
                    );
    ...
    // Now that `args.flags` reflects any per-channel/per-layer rewrites above,
    // recompute the compact op_type used by the ACCX tail epilogue.
    args.op_type =
    ...
    let read_bias = |oc_idx: usize| -> i32 {
        if let Some(b) = bias_bytes {
            let base = oc_idx * 4;
            if base + 4 <= b.len() {
                i32::from_le_bytes([b[base], b[base + 1], b[base + 2], b[base + 3]])
            } else {
                0
            }
        } else {
    ...
    // BlockedN16HWC16 strides (used only for OHWI scalar fallback, not for blocked SIMD border)
    ...
    // Iterate H×W in Rust, call ASM per pixel (esp-dl style). ASM walks KH×KW and OC.
    #[cfg(all(feature = "simd-s3", target_arch = "xtensa"))]
    unsafe {
        // esp-dl TIE728 SIMD requires 16-byte aligned tensor base pointers.
        // The "unaligned" kernel handles byte offsets WITHIN aligned buffers, not misaligned base
        // pointers.
        ...
        for oy in 0..oh {
            for ox in 0..ow {
                ...
                if in_y_origin >= 0 && in_x_origin >= 0 && max_y < ih && max_x < iw {
                    // Fully in-bounds window: use SIMD kernel.
                    #[cfg(feature = "trace")]
                    {
                        simd_pixels += 1;
                    }
        ...
        #[cfg(feature = "trace")]
        {
            ...
            crate::ne_info!(
                "hwcn_unaligned id={}: simd={}px ({}cy, {}cy/px), pad={}px ({}cy, {}cy/px)",
                node.output,
                simd_pixels,
                simd_cycles,
                simd_cy_per_px,
...
fn build_kernel_args_hwcn_unaligned(
    plan: &PlannedArena,
    arena: &mut Arena<'_>,
    spec: &ModelSpec,
    node: &Conv2dSpec,
    ...
    // Per-layer bias table pointer for OC tail path when not per-channel
    ...
    // IC/OC tiling helpers and dilation offsets
    args.c_div_x_1 = (ic / 16) - 1;
    args.c_remainder = ic % 16;
    args.oc_tiles = oc / 16;
    args.oc_tail = oc % 16;

    // Dilation offsets for HWCN
