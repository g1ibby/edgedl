...
use crate::{supported_chips, windows_safe_path};
...
pub fn load_cargo_toml(examples_path: &Path) -> Result<Vec<Metadata>> {
    ...
    for package_path in packages {
        ...
        // Filter to only chip features (skip other features like default, defmt, etc.)
        let chips = toml
            .features
            .keys()
            .filter_map(|key| Chip::from_str(key, true).ok())
            .collect::<BTreeSet<_>>();
