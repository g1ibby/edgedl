...
### "Could not determine flash size" / "Error reading register"
...
This indicates the ESP32's debug interface is stuck. To fix:

1. **Unplug the device** from USB completely
...
If the reset command also times out, try holding the **BOOT button** while plugging in the USB cable, then release and retry the reset command.
