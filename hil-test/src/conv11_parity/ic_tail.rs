...
fn ic_tail_aligned_bases() {
    ...
    if !matches!(simd, ExecOutcome::Executed) {
        edgedl::ne_error!(
            "conv11_ic_tail_parity (aligned): SIMD path fell back; scalar result only (assert will fail)"
        );
    ...
    for i in 0..OUT_ELEMS {
        ...
        if d > 1 {
            edgedl::ne_error!(
                "conv11_ic_tail_parity (aligned): scalar/SIMD mismatch at idx={} scalar={} simd={} (assert will fail)",
                i,
                golden[i],
                got[i]
            );
...
fn ic_tail_misaligned_bases() {
    ...
    if !matches!(simd, ExecOutcome::Executed) {
        edgedl::ne_error!(
            "conv11_ic_tail_parity (misaligned): SIMD path fell back; scalar result only (assert will fail)"
        );
        assert!(matches!(simd, ExecOutcome::Executed));
    }
    ...
    for i in 0..OUT_ELEMS {
        ...
        if d > 1 {
            edgedl::ne_error!(
                "conv11_ic_tail_parity (misaligned): scalar/SIMD mismatch at idx={} scalar={} simd={} (assert will fail)",
                i,
                golden[i],
                got[i]
            );
