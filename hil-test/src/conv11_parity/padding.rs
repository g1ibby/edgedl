use edgedl::{
    ...
    model::{Activation, ModelSpec, NodeOp, NodeSpec},
...
macro_rules! run_padding_test {
    ($case:expr, $pads:expr $(,)?) => {{
        ...
        // Patch the plan to include padding
        ...
        let spec_with_padding = ModelSpec {
        ...
        if !matches!(outcome, ExecOutcome::Executed) {
            edgedl::ne_error!("conv11_padding: SIMD path fell back");
            assert!(
        ...
        for i in 0..OUT_ELEMS {
            ...
            if diff > 1 {
                edgedl::ne_error!(
                    "conv11_padding: mismatch at idx={} scalar={} simd={}",
                    i,
                    golden[i],
                    got[i]
                );
...
pub fn test_padding_1111_linear_pt() {
    run_padding_test!(
        &Case {
...
pub fn test_padding_1111_relu_bias_pt() {
    run_padding_test!(
        &Case {
            layout: super::util::Layout::OHWI,
            w_exps: WeightExps::PerTensor0,
...
pub fn test_padding_1111_linear_pc() {
    run_padding_test!(
        &Case {
            layout: super::util::Layout::OHWI,
...
pub fn test_padding_1111_relu_bias_pc() {
    run_padding_test!(
        &Case {
            layout: super::util::Layout::OHWI,
            w_exps: WeightExps::PerChannel,
...
pub fn test_padding_blocked_relu_bias_pt() {
    run_padding_test!(
        &Case {
