...
fn oc_tail_aligned_output() {
    ...
    if !matches!(simd, ExecOutcome::Executed) {
        edgedl::ne_error!(
            "conv33_oc_tail_parity (aligned): SIMD path fell back; scalar result only (assert will fail)"
        );
    ...
    for i in 0..OUT_ELEMS {
        ...
        if d > 1 {
            edgedl::ne_error!(
                "conv33_oc_tail_parity (aligned): scalar/SIMD mismatch at idx={} scalar={} simd={} (assert will fail)",
                i,
                golden[i],
                got[i]
            );
...
fn oc_tail_misaligned_output() {
    ...
    if !matches!(simd, ExecOutcome::Executed) {
        defmt::info!(
            "conv33_oc_tail_parity (misaligned): SIMD path fell back; scalar result only (assert will fail)"
        );
        edgedl::ne_error!(
            "conv33_oc_tail_parity (misaligned): SIMD path fell back; scalar result only (assert will fail)"
        );
    ...
    for i in 0..OUT_ELEMS {
        ...
        if d > 1 {
            edgedl::ne_error!(
                "conv33_oc_tail_parity (misaligned): scalar/SIMD mismatch at idx={} scalar={} simd={} (assert will fail)",
                i,
                golden[i],
                got[i]
            );
