...
macro_rules! run_padding_test {
    ($case:expr, $pads:expr, $strides:expr, $dilations:expr $(,)?) => {{
        ...
        if !matches!(outcome, ExecOutcome::Executed) {
            edgedl::ne_error!("conv33_padding: SIMD path fell back");
            assert!(
        ...
        for i in 0..OUT_ELEMS {
            ...
            if diff > 1 {
                edgedl::ne_error!(
                    "conv33_padding: mismatch at idx={} scalar={} simd={}",
                    i,
                    golden[i],
                    got[i]
                );
...
pub fn test_padding_3333_linear_pt() {
    run_padding_test!(
        &Case {
...
pub fn test_padding_3333_relu_bias_pt() {
    run_padding_test!(
        &Case {
            layout: super::util::Layout::OHWI,
            w_exps: WeightExps::PerTensor0,
...
pub fn test_padding_3333_relu_bias_pc() {
    run_padding_test!(
        &Case {
            layout: super::util::Layout::OHWI,
